import { Card } from "@/components/ui/card";
import { CheckCircle, File } from "lucide-react";

interface Props {
  fileName?: string;
  // filePath?: string;
}

const MinuteUploadSuccess: React.FC<Props> = ({ fileName }) => {
  return (
    <div className="flex flex-col gap-6 lg:max-w-4/5 xl:max-w-3/5 mx-auto">
      <Card className="shadow-md border-green-500 border-2 p-6">
        <div
          className="flex flex-col items-center justify-center text-center gap-4"
          aria-label="minute-upload-success"
        >
          <CheckCircle className="text-green-600 w-16 h-16" />
          <h2 className="text-green-700 text-3xl font-semibold">
            Minute सफलतापूर्वक अप्लोड भयो
          </h2>
          <h3 className="text-muted-foreground text-2xl">
            कृपया अब फारम भर्नुहोस्
          </h3>
          {fileName && (
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              <File className="w-4 h-4 text-green-600" />
              <span className="truncate max-w-sm">{fileName}</span>
            </div>
          )}
        </div>
      </Card>
    </div>
  );
};

export default MinuteUploadSuccess;
